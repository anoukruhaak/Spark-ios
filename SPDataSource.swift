...
class SPDataSource: NSObject, UITableViewDataSource, UIWebViewDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
       ...
       let newHTMLString = "<style> img{max-width:300px} code, pre {max-width:300px; background:yellow} blockquote{font-style:italic} h1, h2, h3, body{max-width:300px; font: 16px \"Helvetica\";} strong{color: green} h1{font-size: 26px;} h2{font-size: 22px;} h3{font-size: 18px;}</style>\(baseHTMLString)"
        NSLog("\(newHTMLString)")
